  ...
  return (
    <section className="py-32 bg-zinc-950 relative border-t border-white/5">
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <Reveal stagger delay={0.1} className="grid md:grid-cols-3 gap-8">
          {reasons.map((reason, index) => (
            <RevealItem key={index} className="bg-background rounded-2xl p-8 border border-white/5 hover:border-primary/30 transition-colors group">
              <div className="w-14 h-14 rounded-xl bg-zinc-900 border border-white/10 flex items-center justify-center text-primary-dim mb-6 group-hover:bg-primary/10 group-hover:text-primary group-hover:rotate-6 transition-all duration-500">
                {reason.icon}
              </div>
